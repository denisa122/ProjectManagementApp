...
const updateUser = async (req, res) => {
  ...
  if (req.user && req.user.id === userId) {
    try {
      ...
      res
        .status(200)
        .send({ message: "User was updated successfully.", userUpdated });
    } catch (error) {
      res
        .status(500)
        .send({ message: `Cannot update user.`, error: error.message });
    }
  } else {
    res
      .status(403)
  ...
  if (req.user && req.user.id === userId) {
    try {
      ...
      if (!deletedUser) {
        res
          .status(404)
          .send({ message: `Cannot delete user. User not found.` });
      } else {
      ...
      res
        .status(500)
        .send({ message: `Cannot delete user.`, error: error.message });
    }
  } else {
    res
      .status(403)
